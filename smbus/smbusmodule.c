...
PyDoc_STRVAR(B16_read_byte_data_doc,
	"read_byte_data_16b(addr, cmd) -> result\n\n"
	"Perform 16-bit Read Byte Data transaction.\n");

static PyObject *
B16_read_byte_data(SMBus *self, PyObject *args)
{
	int addr, cmd;
	struct i2c_msg msgs[2];
	struct i2c_rdwr_ioctl_data data;
        char wrbuf[64], rdbuf[64];

	if (!PyArg_ParseTuple(args, "ii:read_byte_data_16b", &addr, &cmd))
		return NULL;

	wrbuf[0] = (cmd >> 8) & 0x00FF;
	wrbuf[1] = cmd & 0x00FF;
	...
	if (ioctl(self->fd, I2C_RDWR, &data) != 2) {
		PyErr_SetFromErrno(PyExc_IOError);
		return NULL;
	}
	...
	return Py_BuildValue("l", (long)(rdbuf[0]));
}
...
PyDoc_STRVAR(B16_write_byte_data_doc,
	"write_byte_data_16b(addr, cmd, val)\n\n"
	"Perform 16-Bit Write Byte Data transaction.\n");

static PyObject *
B16_write_byte_data(SMBus *self, PyObject *args)
{
	int addr, cmd, val;
	...
	if (!PyArg_ParseTuple(args, "iii:write_byte_data_16b", &addr, &cmd, &val))
		return NULL;
	...
	if (ioctl(self->fd, I2C_RDWR, &data) != 1) {
		PyErr_SetFromErrno(PyExc_IOError);
		return NULL;
	}
 ...
 */
static int
SMBus_list_to_buf(PyObject *list, char *data)
{
	static char *msg = "Third argument must be a list of at least one, "
				"but not more than 32 integers";
	int ii, len;
...
PyDoc_STRVAR(B16_read_i2c_block_data_doc,
	"read_i2c_block_data_16b(addr, cmd, len=32) -> results\n\n"
	"Perform 16-Bit I2C Block Read transaction.\n");

static PyObject *
B16_read_i2c_block_data(SMBus *self, PyObject *args)
{
	int addr, cmd, len=32;
	...
	if (!PyArg_ParseTuple(args, "ii|i:read_i2c_block_data_16b", &addr, &cmd,
			&len))
		return NULL;
	...
	return SMBus_buf_to_list(rdbuf, len);
}
...
PyDoc_STRVAR(B16_write_i2c_block_data_doc,
	"write_i2c_block_data_16b(addr, cmd, [vals])\n\n"
	"Perform I2C 16-Bit Block Write transaction.\n");

static PyObject *
B16_write_i2c_block_data(SMBus *self, PyObject *args)
{
	int addr, cmd, len;
	...
	if (!PyArg_ParseTuple(args, "iiO&:write_i2c_block_data_16b", &addr, &cmd,
				SMBus_list_to_buf, &(wrbuf[1])))
		return NULL;
...
static PyMethodDef SMBus_methods[] = {
	...
	{"read_byte_data_16b", (PyCFunction)B16_read_byte_data, METH_VARARGS,
		B16_read_byte_data_doc},
	{"write_byte_data", (PyCFunction)SMBus_write_byte_data, METH_VARARGS,
		SMBus_write_byte_data_doc},
	{"write_byte_data_16b", (PyCFunction)B16_write_byte_data, METH_VARARGS,
		B16_write_byte_data_doc},
	...
	{"read_i2c_block_data_16b", (PyCFunction)B16_read_i2c_block_data,
		METH_VARARGS, B16_read_i2c_block_data_doc},
	{"write_i2c_block_data", (PyCFunction)SMBus_write_i2c_block_data,
		METH_VARARGS, SMBus_write_i2c_block_data_doc},
	{"write_i2c_block_data_16b", (PyCFunction)B16_write_i2c_block_data,
		METH_VARARGS, B16_write_i2c_block_data_doc},
	{NULL},
};
